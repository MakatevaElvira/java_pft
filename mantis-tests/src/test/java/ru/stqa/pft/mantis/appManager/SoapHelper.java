...
public class SoapHelper {
  ...
  public Set<Project> getProjects() throws MalformedURLException, ServiceException, RemoteException {
    ...
    return Arrays.asList(projects).stream()
            .map((p) -> new Project().withId(p.getId().intValue()).withName(p.getName())) //преобразовали БигИнтеджер в ИНТ(withId(p.getId().intValue()))
  ...
  public Issue addIssue(Issue issue) throws MalformedURLException, ServiceException, RemoteException {
    ...
    IssueData issueData = new IssueData(); //Багрепорт
    issueData.setSummary(issue.getSummary());
    issueData.setDescription(issue.getDescription());
    issueData.setProject(new ObjectRef(BigInteger.valueOf(issue
            .getProject().getId()),issue.getProject().getName())); //тут сами посторили Объект типа BigInteger
    issueData.setCategory(categories[0]); //выбираем первую попавшуюся категорию

    BigInteger issuerId = mc.mc_issue_add( //=ИД созданного Багрепорта
            app.getProperty("web.adminLogin"), app.getProperty("web.adminPassword"), issueData);
    IssueData createdIssueData = mc.mc_issue_get(app.getProperty("web.adminLogin"), app.getProperty("web.adminPassword"), issuerId);
  ...
  public String getStatus(int issueId) throws MalformedURLException, ServiceException, RemoteException {
    MantisConnectPortType mc = getMantisConnect();
    String status = String.valueOf(mc.mc_issue_get(app.getProperty("web.adminLogin"),
